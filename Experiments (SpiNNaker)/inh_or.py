...
if __name__ == '__main__':
    ...
    input_times = [np.random.randint(10, int(global_params["sim_time"] - 10), int(global_params["sim_time"] * 0.2)) for i in range(n_bits)]
    ...
    not_pop = sim.Population(1, sim.IF_curr_exp(**neuron_params), initial_values={'v': neuron_params["v_rest"]}, label="not")

    # - Connections -
    # OP
    sim.Projection(op_pop, delay_pop, sim.OneToOneConnector(), std_conn)
    sim.Projection(op_pop, nor_pop, sim.OneToOneConnector(), std_conn)

    # Inputs to NOR
    for i in range(n_bits):
        sim.Projection(input_pop[i], nor_pop, sim.OneToOneConnector(), std_conn, receptor_type="inhibitory")

    # NOT
    sim.Projection(delay_pop, not_pop, sim.OneToOneConnector(), std_conn)
    sim.Projection(nor_pop, not_pop, sim.OneToOneConnector(), std_conn, receptor_type="inhibitory")
    ...
    plt.plot(np.array(nor_data.spiketrains[0]) - 1, [n_bits] * len(nor_data.spiketrains[0]), 'o', markersize=0.5, color='tab:red')
    plt.plot(np.array(not_data.spiketrains[0]) - 2, [n_bits+1] * len(not_data.spiketrains[0]), 'o', markersize=0.5, color='tab:green')

    plt.xlabel("Time (ms)")
    plt.ylabel("Neurons")
    plt.yticks(range(-2, n_bits + 3), ["", "OP"] + ["Input " + str(i) for i in range(n_bits)] + ["NOR (-1)", "NOT (-2)", ""])
